...
namespace gallt {
    ...
    bool GenericExpander::substitute_type(const Type& in, const Substitution& sub, Type& out) {
        switch (in.kind) {
        case TypeKind::Array: {
            if (in.element_type && in.element_type->kind == TypeKind::Struct &&
                !in.element_type->generic_ref && in.array_size.has_value()) {
                auto pack = sub.type_packs.find(in.element_type->struct_name);
                if (pack != sub.type_packs.end()) {
                    const std::size_t index = *in.array_size;
                    if (index >= pack->second.size()) {
                        report(SourceLocation{},
                            ErrorCode::SubscriptOutOfBounds,
                            std::vector<std::string>{ std::to_string(index),
                                std::to_string(pack->second.size()) });
                        out = Type::make_void();
                        return false;
                    }
                    out = pack->second[index];
                    return true;
                }
            }
