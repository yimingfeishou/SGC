// semantic/constant_folding.cpp
// Gallt 0.3.txt §19：编译期常量表达式求值实现
// Gallt 0.3.txt §19: compile-time constant expression evaluation
...
namespace gallt {
    ...
    namespace {
        // 解析字符字面量的取值（支持简单转义）
        // Decode a character literal value (simple escapes)
        ...
        // 按目标类型截断/提升（第 7 章：右值精度高于左值时截断）
        // Truncate/promote into the target type (§7)
        ...
        Value evaluate(const Expression* expr, const ConstantEvaluationContext& ctx) {
            ...
            if (auto* prim = dynamic_cast<const PrimaryExpression*>(expr)) {
                ...
                case PrimaryExpression::Kind::Literal: {
                    ...
                    return Value{};   // 字符串字面量不是数值型编译期常量
                }
                case PrimaryExpression::Kind::Identifier: {
                    // 已实例化的编译期常量参数（第 19 章）
                    // A bound compile-time constant parameter (§19)
    ...
    bool evaluate_constant_expression(const AST::Expression* expr, long long& int_out,
        ...
        if (!ctx.type_layout) {
            // 至少支持内建类型的大小/对齐
            // At minimum, builtin type layouts are always available
